...
const io = new Server(server, {
  cors: {
    ...
    allowedHeaders: ["Content-Type", "Authorization", "X-Requested-With", "x-api-key", "x-business-id", "x-agency-clearance", "x-did-proof", "x-encrypted"]
  }
});
...
app.get('/api/admin/lan-traffic-logs', enforceTenantIsolation, (req, res) => {
    ...
    return ok(res, {
        success: true,
        activeNodeIp: req.socket.localAddress,
        totalCapturedPackets: lanTrafficLogs.length,
        lanTrafficLogs: lanTrafficLogs.slice(-50).reverse()
    });
});
