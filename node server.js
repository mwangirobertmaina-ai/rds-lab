...
app.post("/addBusiness", addBusinessHandler);
...
const addProductHandler = (req, res) => {
  try {
    ...
    if (!name || price === undefined) {
      return fail(res, "Missing required product parameters: name, price");
    }
...
app.post("/product/add", addProductHandler);
...
const checkoutHandler = (req, res) => {
  try {
    ...
    let calculatedTotal = num(amount || total);
    let calculatedCommission = 0;
    const itemsList = Array.isArray(items) ? items : [];

    if (itemsList.length > 0 && calculatedTotal === 0) {
      itemsList.forEach(item => {
        const product = data.products.find(p => p.id === (item.productId || item.id));
        const price = product ? product.price : num(item.price);
        const qty = num(item.qty || item.quantity || 1);
        const sub = price * qty;
        calculatedTotal += sub;
        calculatedCommission += Math.floor(sub * 0.05);
      });
    }
    ...
    const order = {
      id: id("ORD"),
      businessId: businessId || "SYSTEM",
      customerName: (customerName || "Guest Customer").trim(),
    ...
    log("ORDER", `Created: ${order.id} ($${calculatedTotal})`);
...
app.post("/orders/create", checkoutHandler);
...
const completeDeliveryHandler = (req, res) => {
  try {
    ...
    if (!orderId) return fail(res, "orderId or deliveryId is required");

    let delivery = data.deliveries.find(d => d.id === orderId || d.orderId === orderId);
    let order = data.orders.find(o => o.id === orderId || (delivery && o.id === delivery.orderId));

    if (!order && !delivery) return fail(res, "Order or delivery record not found", 404);
    ...
    if (targetDriverId) {
      ...
      if (driver) {
        const payout = order ? (order.total * 0.90) : 50;
    ...
    io.emit("order:completed", { orderId: order ? order.id : orderId, driverId: targetDriverId });
    log("FULFILLMENT", `Completed: ${order ? order.id : orderId}`);
    ok(res, { message: "Delivery completed successfully", order, delivery });
  } catch (err) {
    fail(res, "Completion processing failed", 500);
  }
};

app.post("/completeDelivery", completeDeliveryHandler);
app.post("/order/complete", completeDeliveryHandler);
app.post("/orders/complete", completeDeliveryHandler);
...
app.get("/driverJobs", (req, res) => {
  try {
    ...
    if (driverId) {
      list = list.filter(d => d.driverId === driverId);
    }

    ok(res, { success: true, jobs: list, deliveries: list });
  } catch (err) {
    fail(res, "Failed to load driver jobs", 500);
  }
});
...
app.get("/deliveries", (req, res) => {
  try {
    ...
    fail(res, "Failed to load deliveries", 500);
  }
});
...
/* ================= SERVER START ================= */
server.listen(PORT, () => {
  log("SYSTEM", `🚀 RDS ULTRA CORE SERVER RUNNING ON PORT ${PORT} (SOCKET_ENABLED)`);
});